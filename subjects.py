...
# пофиксить blocks и blocklist и сделать что-то одно


class MainPlayer(pygame.sprite.Sprite):  # class about actions of main player
    def __init__(self, blocksdict, width_window, height_window, win, blocks, maingunlist):
    ...
    def orientation(self, angle: int):  # ротирует изображение исходя из угла полученного в параметре angle
        ...
        self.win.blit(rotated_image, (self.rect.centerx, self.rect.centery))  # отрисовка разворота игрока
        return None
    ...
    def moving(self, keys):  # передвигает объект игрока на экране согласно ограничений по недоступным местам(блокам)
        if keys[pygame.K_a] and self.rect.centerx > 20 and self.blocks(self.rect.centerx, self.rect.centery, 'left',
                                                                  self.speed):
        ...
        if keys[pygame.K_d] and self.rect.centerx < self.width_window - 20 and self.blocks(self.rect.centerx, self.rect.centery,
                                                                                 'right', self.speed):
            self.rect.centerx += self.speed
        ...
        if keys[pygame.K_w] and self.rect.centery > 20 and self.blocks(self.rect.centerx, self.rect.centery, 'up',
                                                                  self.speed):
        ...
        if keys[pygame.K_s] and self.rect.centery < self.height_window - 20 and self.blocks(self.rect.centerx, self.rect.centery,
                                                                                  'down', self.speed):
            self.rect.centery += self.speed
    ...
    def startshooting(self, keys):
        if keys[pygame.K_LCTRL]:
            if self.upflag_Orientation:
                self.maingunlist.append(MainPlayerGun(self.rect.centerx, self.rect.centery, 2, RED, 50, "up"))
            if self.downflag_Orientation:
                self.maingunlist.append(MainPlayerGun(self.rect.centerx, self.rect.centery, 2, RED, 50, "down"))
            if self.rightflag_Orientation:
                self.maingunlist.append(MainPlayerGun(self.rect.centerx, self.rect.centery+34, 2, RED, 50, "right"))
            if self.leftflag_Orientation:
                self.maingunlist.append(MainPlayerGun(self.rect.centerx, self.rect.centery+34, 2, RED, 50, "left"))
